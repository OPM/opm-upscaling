...
#include <dune/porsol/blackoil/co2fluid/opm/common/valgrind.hh>
...
public:
    ...
    Scalar massFraction(int phaseIdx, int compIdx) const
    {
        return
            ...
            * FluidSystem::molarMass(compIdx)
            / std::max(1e-40, averageMolarMass_[phaseIdx]);
    }
    ...
    {
        averageMolarMass_[phaseIdx] = compo.meanMolarMass();
        pressure_[phaseIdx] = compo.pressure();

        //phaseConcentration_[phaseIdx] = compo.phaseConcentration();
        
        density_[phaseIdx] = compo.density();
        sumMoleFractions_[phaseIdx] = 0.0;
        for (int compIdx = 0; compIdx < numComponents; ++compIdx) {
            //concentration_[phaseIdx][compIdx] = compo.concentration(compIdx);
            sumMoleFractions_[phaseIdx] += compo.moleFrac(compIdx);
            moleFraction_[phaseIdx][compIdx] = compo.moleFrac(compIdx);
        };
    };
