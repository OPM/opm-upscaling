...
class IRISDuneISTLInterface
{
  public:
    ...
    template<class IRISGrid>
    void setupMatrix(IRISGrid& grid, const int& codim);
...
template<class IRISGrid>
void IRISDuneISTLInterface::setupMatrix(IRISGrid& grid, const int& codim)
{
  if (codim == 0)
  {
    // Generate look-up table of element to element neighbour relations ...
    std::vector<std::set<int> > elm2elmIndx(nRow_);
    for (typename IRISGrid::ElementIterator it = grid.template setEntityPointerToFirst<typename IRISGrid::ElementIterator>(0); 
	                                it != grid.template entityPointerIsAtEnd<typename IRISGrid::ElementIterator>(0); ++it)
	 {
